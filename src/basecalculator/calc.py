...
from .settings import error_text
...
def factorial_expr(expr: str) -> Tuple[int, str]:
    """
    Выбор выражения под знаком факториала.
...
def calculate(expr: str) -> str:
    ...
    try:
        ...
        result = error_text()["value"]
    except ZeroDivisionError:
        result = error_text()["zero_division"]
    except OverflowError:
        result = error_text()["overflow"]
    except (SyntaxError, NameError):
        result = error_text()["syntax"]
    except TypeError:
        result = ""
    return result
